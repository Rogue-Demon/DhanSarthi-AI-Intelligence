...
async def test_ai_advisor_intent_detection():
    ...
    # 3. Ask about commodity (silver/gold)
    data = await service._retrieve_live_market_data("Do we invest in silver, tell the price", 999)
    assert data is not None
    assert "commodities" in data
    assert data["commodities"][0]["symbol"] == "XAGUSD"

    # 4. Ask about stock in natural language ("price of reliance")
    data = await service._retrieve_live_market_data("Tell me the price of reliance stock", 999)
    assert data is not None
    assert "stocks" in data
    assert data["stocks"][0]["symbol"] == "RELIANCE.NS"
