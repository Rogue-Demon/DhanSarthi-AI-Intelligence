...
class ResponseQualityEvaluator:
    ...
    def evaluate(
        ...
        if not response_text or not response_text.strip():
            ...
            # Generic AI refusal detection when context is present
            refusal_patterns = [
                r"i\s+can'?t\s+provide\s+(real-time|current|live|\w+)",
                r"i\s+cannot\s+provide\s+(real-time|current|live|\w+)",
                r"i'?m\s+not\s+able\s+to\s+provide\s+(real-time|current|live|\w+)",
                r"don'?t\s+have\s+real-time\s+(access|data|information)",
                r"not\s+within\s+the\s+scope\s+of\s+(our\s+conversation|the\s+provided\s+context)",
                r"not\s+within\s+the\s+provided\s+context",
            ]
            has_context = bool(
                (ai_context and getattr(ai_context, "live_market_data", None))
                or (ai_context and getattr(ai_context, "user_financial_context", None))
                or retrieved_docs
            )
            if has_context:
                for rpat in refusal_patterns:
                    if re.search(rpat, text_strip, re.IGNORECASE):
                        completeness_score = 0.0
                        failure_reasons.append(f"GENERIC_AI_REFUSAL: LLM returned generic refusal '{rpat}' despite available context.")
                        retry_hints.append("Answer the question directly using the provided facts and Live Market Data.")
