...
class AIAdvisorService:
    ...
    async def send_chat_message(
        ...
        # Determine whether user financial data was actually present AND used
        has_personal_data = bool(
            ai_context.user_financial_context and 
            self._builder._has_any_financial_data(ai_context.user_financial_context)
        )
        used_user_financial_data = (
            has_personal_data and (
                intent in (QueryIntent.PERSONAL_FINANCE, QueryIntent.MIXED) or 
                getattr(understanding, "requires_personal_data", False)
            )
        )

        # Check for calculation details (commodity conversions, health scores, portfolio estimations)
        calc_details = []
        calculation_performed = False

        if live_market_data:
            # Check for commodity conversion calculations
            comms = live_market_data.get("commodities") or []
            for item in comms:
                if isinstance(item, dict) and item.get("conversion"):
                    calculation_performed = True
                    calc_details.append({
                        "type": "commodity_conversion",
                        "title": f"{item.get('name')} Price Conversion",
                        "symbol": item.get("symbol"),
                        "formula": item["conversion"].get("formula"),
                        "inputs": item["conversion"].get("inputs"),
                        "timestamp": item.get("timestamp"),
                        "source": item.get("source"),
                    })

            # Check for portfolio estimation calculations
            if live_market_data.get("portfolio_estimation"):
                pe = live_market_data["portfolio_estimation"]
                calculation_performed = True
                calc_details.append({
                    "type": "portfolio_estimation",
                    "title": "Portfolio Live Valuation",
                    "formula": "Difference = Estimated Value (Live Market Quotes) - Stored Value",
                    "inputs": {
                        "stored_value": float(pe.get("total_stored_value", 0)),
                        "estimated_value": float(pe.get("total_estimated_value", 0)),
                        "difference": float(pe.get("difference", 0)),
                        "difference_percent": float(pe.get("difference_percent", 0)),
                    }
                })

        if health_score_meta and health_score_meta.get("breakdown") and used_user_financial_data:
            calculation_performed = True
            for item in health_score_meta["breakdown"]:
                calc_details.append({
                    "type": "health_score",
        ...
        assistant_metadata = {
            ...
            "language": understanding.language,
            "correction_applied": understanding.correction_applied,
            "data_completeness": data_quality_meta if used_user_financial_data else None,
            "used_user_financial_data": used_user_financial_data,
            "calculation_performed": calculation_performed,
            "calculation_details": calc_details if calculation_performed else None,
            "signals": signals_meta if used_user_financial_data else [],
            "health_score": health_score_meta if used_user_financial_data else None,
    ...
    async def _retrieve_live_market_data(
        ...
        # Popular stock name -> symbol mapping
        stock_name_map = {
        ...
        commodities = []
        seen_coms = set()
        for com_name, com_symbol in commodity_map.items():
            if com_name in q and com_symbol not in seen_coms:
                seen_coms.add(com_symbol)
                com_quote = await self._market.get_commodity_quote(com_symbol, target_currency="INR")
                commodities.append(com_quote)
        if commodities:
            live_data["commodities"] = commodities

        # 2. Check if user is asking about portfolio value
        ...
        # 3. Extract FX rates (e.g. USD to INR, EUR/INR)
        ...
        # 4. Extract mutual fund scheme IDs (5 to 8 digit numbers)
        ...
        # 5. Extract stock symbols (uppercase, e.g. RELIANCE.NS, TCS, MSFT) or mapped stock names
        exclude_set = {"USD", "INR", "EUR", "RAG", "SIP", "NAV", "DTI", "FD", "RD", "EMI", "AI", "BSE", "NSE", "GDP"}
        stock_symbols = set()
        
        # Check stock name map
        for sname, ssym in stock_name_map.items():
            if sname in q:
                stock_symbols.add(ssym)

        # Check uppercase regex matches
        ...
        stocks = []
        for sym in list(stock_symbols)[:3]:
            try:
    ...
    async def _evaluate_and_retry_if_needed(
        ...
        # 4. Safe Grounded Fallback if initial and retry both fail
        if ai_context and getattr(ai_context, "live_market_data", None):
            lmd = ai_context.live_market_data
            market_parts = []
            if lmd.get("commodities"):
                for c in lmd["commodities"]:
                    if c.get("available"):
                        disp = c.get("display", {})
                        orig = c.get("original", {})
                        market_parts.append(
                            f"The current price of {c.get('name')} is {disp.get('currency')} {disp.get('price'):,.2f} per {disp.get('unit')} "
                            f"(Original Quote: {orig.get('currency')} {orig.get('price'):,.2f} / {orig.get('unit')})."
                        )
            if lmd.get("stocks"):
                for s in lmd["stocks"]:
                    market_parts.append(
                        f"The current stock price of {s.get('symbol')} ({s.get('name', s.get('symbol'))}) is {s.get('currency', 'INR')} {s.get('price'):,.2f}."
                    )
            if market_parts:
                fallback_text = "\n".join(market_parts)
            elif requires_personalization:
                fallback_text = (
