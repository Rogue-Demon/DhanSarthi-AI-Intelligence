...
class ConversationService:
    ...
    def update_title_from_first_message(
        ...
        if not conversation.title or conversation.title in ("New Conversation", "Untitled Chat", "New Chat", "Untitled"):
    ...
    def store_user_message(self, conversation_id: int, content: str) -> ConversationMessage:
        ...
        If the last message in this conversation is already an identical USER message,
        reuse it to prevent duplicate insertions from stream retries/fallbacks.
        """
        ...
        if recent:
            ...
            if last_role.upper() == "USER" and last.content.strip() == content.strip():
                return last
