...
class MockMarketDataProvider(
    ...
    async def get_quote(self, symbol: str, exchange: Optional[str] = None) -> StockQuote:
        ...
        elif "SILVER" in symbol_upper or "XAG" in symbol_upper:
            price = Decimal("32.50")  # USD per troy oz
            change = Decimal("0.45")
            change_pct = Decimal("1.40")
            name = "Silver Commodity"
            curr = "USD"
        elif "GOLD" in symbol_upper or "XAU" in symbol_upper:
            price = Decimal("2500.00")  # USD per troy oz
            change = Decimal("12.50")
            change_pct = Decimal("0.50")
            name = "Gold Commodity"
            curr = "USD"
        elif "CRUDE" in symbol_upper or "USOIL" in symbol_upper:
            price = Decimal("6350.00")
            change = Decimal("-80.00")
            change_pct = Decimal("-1.24")
            name = "Crude Oil Commodity"
            curr = "INR"
        elif "PLATINUM" in symbol_upper or "XPT" in symbol_upper:
            price = Decimal("32100.00")
        ...
        return StockQuote(
            symbol=symbol,
            exchange=exchange or "COMMODITY/NSE",
            price=price,
            currency=curr,
            timestamp=now,
